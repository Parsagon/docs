---
title: 'Quick Search'
description: 'Execute a search and receive results immediately, without needing to poll a persistent Search object for completion.'
...
<ParamField body="test_data" type="object">
  Custom dummy data to return when `test` is `true`. Quick Search only supports custom `search_results`; it does not support custom edited outputs.
...
<ResponseField name="search_results" type="array">
  ...
  <Expandable title="Search result fields">
    ...
    <ResponseField name="search_topics" type="array">
...
Use `test: true` to test your integration without running a source search. The endpoint still validates the request body the same way it does for a normal Quick Search request.
