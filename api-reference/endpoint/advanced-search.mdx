...
<ParamField body="edit_prompt" type="string">
  Instructions for formatting or transforming the search results into `edited_results`. Maximum 5000 characters.
</ParamField>
...
<ParamField body="test_data" type="object">
  Custom dummy data to store when `test` is `true`. Use this to test downstream handling of `search_results` or custom `edited_results` formats.
...
Use `test: true` to test your integration without running a source search. The endpoint still validates the request body the same way it does for a normal Advanced Search request.

If you include `test_data.edited_results`, the value is returned by [Get Search](/api-reference/endpoint/get-search) as the Search object's `edited_results`. The API does not run `edit_prompt` in test mode.
